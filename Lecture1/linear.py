...
x = linspace(0, 1, 500) # Генерира 500 числа на равни разстояния и ги записва в масив - х-овете
y = x * 2 + 1 + random.normal(size = 500) / 3 # модел, random.normal(size) / а е разпределяща функция, която изчислява вероятността да има число на разстояние от 0 - колкото е по - далеч, толкова е по - малко вероятно да го има като при скалата с оценките масово ще има четворки, но много рядко двойки и шестици - у-ците
